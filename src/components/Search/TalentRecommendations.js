...
//Modified Apply button color and status Code
...
const TalentRecommendationsItems = () => {
  ...
  const [appliedJobIds, setAppliedJobIds] = useState([]);  //Response  from API of appliedJobList  should be stored here stored Here
  ...
  // Update the appliedJobIds array when the "Apply" button is clicked  
  const mailSenderToRecruiter = async (recruiterId) => {
    console.log(recruiterId);
    fetch(`${baseurl}/sendMail?id=${recruiterId}&senderId=${user._id}`)
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
        setAppliedJobIds([...appliedJobIds, recruiterId]);
        ...
        {recommendedJobs?.map((job) => (
          <Card key={job.id} sx={{ minWidth: 250, marginBottom: '20px' }}>
            ...
            <CardActions>
              <Button
                size="small"
                onClick={() => handleToggleJobDescription(job)}
              >
                Job Description
              </Button>
              ...
              <Button
                 ...
                 color={appliedJobIds.includes(job._id) ? 'error' : 'secondary'}
                 onClick={() => mailSenderToRecruiter(job._id)}
