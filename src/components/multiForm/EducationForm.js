...
// import { yearofPassouts, educationLevels, authorities ,disciplines } from '../../constraints/arrays';
...
export default function EducationForm() {
  ...
  function SaveEducation() {
    const saveRequests = educationList.map((e) => {
      return fetch(`${baseurl}/education`, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const saveResults = await SaveEducation();

      // Check if all education entries were successfully saved
      const allSavedSuccessfully = saveResults.every(result => result.status !== false);

      if (allSavedSuccessfully) {
        // Clear the educationList and navigate to the next page (ExperienceForm)
        setEducationList([initialEducation]);
        navigate("/ExperienceForm");
      } else {
        // Handle the case where some education entries failed to save
        alert("Some education entries failed to save. Please try again.");
      }
    } catch (error) {
      // Handle any errors that occur during saving
      console.error("Error while saving education entries:", error);
      alert("An error occurred while saving education entries. Please try again later.");
    }
  };
  ...
  return (
    <Container>
      ...
      {educationList.map((education, i) => (
        <Paper key={i} style={{ border: '1px solid', marginTop: '60px', borderRadius: '0.5rem', padding: '4rem' }}>
          <Typography variant="h6" gutterBottom>
            Education
          </Typography>
          <Grid container spacing={3}>
